...
static int raw_tp_tx_process(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (esp_is_tx_queue_paused(priv)) {
			...
			if (!tx_skb) {
				printf("%s:%u esp_alloc_skb failed\n",__func__,__LINE__);
				msleep(10);
				continue;
			}
	...
	printf("esp32: raw tp tx thrd stopped\n");
	return 0;
}
...
static void process_raw_tp_flags(void)
{
	...
	if (test_raw_tp) {
		...
		if (test_raw_tp__host_to_esp) {
			...
			if (!raw_tp_tx_thread)
				printf( "Failed to create send traffic thread\n");
...
{
#if TEST_RAW_TP
	printf("ESP peripheral test capabilities: 0x%x\n", cap);
