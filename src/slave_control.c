		...
		int32_t event_id, void *event_data)
{
	...
	if (event_id == WIFI_EVENT_AP_STACONNECTED) {
		...
		send_event_data_to_host(CTRL_MSG_ID__Event_StationDisconnectFromESPSoftAP, event->mac, MAC_LEN);
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	do {
		...
		if (ret) {
			//ESP_LOGI(TAG, "Failed to connect to SSID:'%s', password:'%s'", req->req_connect_ap->ssid ? req->req_connect_ap->ssid : "(null)", req->req_connect_ap->pwd ? req->req_connect_ap->pwd : "(null)");
		}
		...
		if (bits & WIFI_CONNECTED_BIT) {
			//ESP_LOGI(TAG, "connected to ap SSID:'%s', password:'%s'", req->req_connect_ap->ssid ? req->req_connect_ap->ssid :"(null)", req->req_connect_ap->pwd ? req->req_connect_ap->pwd :"(null)");
			station_connected = true;
			esp_wifi_internal_reg_rxcb(ESP_IF_WIFI_STA, (wifi_rxcb_t) wlan_sta_rx_callback);
			break;
		} else {
			if (bits & WIFI_NO_AP_FOUND_BIT) {
				//ESP_LOGI(TAG, "No AP available as SSID:'%s'", req->req_connect_ap->ssid ? req->req_connect_ap->ssid : "(null)");
				resp_payload->resp = CTRL__STATUS__No_AP_Found;
			} else if (bits & WIFI_WRONG_PASSWORD_BIT) {
				//ESP_LOGI(TAG, "Password incorrect for SSID:'%s', password:'%s'", req->req_connect_ap->ssid ? req->req_connect_ap->ssid : "(null)", req->req_connect_ap->pwd ? req->req_connect_ap->pwd :"(null)");
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	for (int i = 0; i < credentials.count; i++ ) {
		...
		//ESP_LOGI(TAG,"\nSSID      \t\t%s\nRSSI      \t\t%d\nChannel   \t\t%d\nBSSID     \t\t%s\nAuth mode \t\t%d\n",
#endif
		// results[i]->ssid.data, results[i]->rssi, results[i]->chnl, results[i]->bssid.data, results[i]->sec_prot);
		vTaskDelay(1);
		...
		CtrlMsg *resp, void *priv_data)
{
	...
	if (ret) {
		//ESP_LOGE(TAG, "Mac address not recognized from %s", (char *)req->req_set_mac_address->mac.data);
		goto err;
	}
