	...
	unsigned long event, void *data)
{
	...
	case NETDEV_UP:
		if (priv && (priv->if_type == ESP_STA_IF)) {
			...
			printf("%s: NETDEV_UP interface %s ip changed to  %pi4 \n",
					__func__, netdev->name, &ifa->ifa_local);
		}
		break;

	case NETDEV_DOWN:
		printf("Interface Down: %d\n", priv->if_type);
		if (priv && (priv->if_type == ESP_STA_IF))
...
/*	struct wireless_dev *wdev = NULL;*/
	...
	if (NL80211_IFTYPE_STATION == type) {
		...
		printf("%s:%u network type[%u] is not supported\n",
